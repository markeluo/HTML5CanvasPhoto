...
namespace PhotoApp.BLL
{
    ...
    {
        private static ImgManager manager = new ImgManager();
        /// <summary>
        /// 图片处理 实例 
        /// </summary>
        ...
        /// <returns></returns>
        public string GetImgData(string _ImgPath, bool IsDelete)
        {
            string strImgData = "";
            FileStream fs = null;
            try
            {
                fs = new FileStream(_ImgPath, FileMode.Open, FileAccess.Read, FileShare.Read, 20480, true);
                byte[] imgb = new byte[fs.Length];
                fs.Read(imgb, 0, (int)fs.Length);
                strImgData = Convert.ToBase64String(imgb);
            }
            catch (Exception ex)
            {
            ...
            strImgData = "data:image/jpeg;base64," + strImgData;
            return strImgData;
        }
